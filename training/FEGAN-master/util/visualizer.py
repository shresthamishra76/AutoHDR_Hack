...
class Visualizer():
    def __init__(self, opt):
        ...
        if getattr(opt, 'results_root', '') and opt.results_root:
            results_out = os.path.join(opt.results_root, opt.name)
            os.makedirs(results_out, exist_ok=True)
            self.log_name = os.path.join(results_out, 'loss_log.txt')
            self.img_dir = results_out
        else:
