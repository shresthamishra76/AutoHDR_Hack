...
class GcGANCrossModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.chess = transform1(img).unsqueeze(0).to(self.device)
    ...
    def rot(self, deg):
        ...
        def callback(tensor):
            ...
            inv_idx = torch.arange(size-1, -1, -1).long().to(self.device)
            _iter = deg // 90
    ...
    def forward_G_basic(self, netG, real):
        ...
        self.grid = F.affine_grid(self.theta, self.input_A.shape, align_corners=True).to(self.device) 
        fake = F.grid_sample(real, flow + self.grid, padding_mode="zeros", align_corners=True)
        return (fake, flow)
    ...
    def radial_constraint(self, flow):
        ...
        v = torch.cat((y.unsqueeze(2), x.unsqueeze(2)), dim = 2).unsqueeze(0).repeat(n, 1, 1, 1).to(flow.device)
    ...
    def circle_constraint(self, flow):
        ...
        for r in rad_list:
            X = (r * np.cos(deg_list) + offset).astype(int)
            Y = (r * np.sin(deg_list) + offset).astype(int)
    ...
    def get_current_visuals(self):
        ...
        chess_A = F.grid_sample(self.chess, (self.flow_A + self.grid)[0].unsqueeze(0), align_corners=True)
