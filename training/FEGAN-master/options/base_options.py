...
class BaseOptions():
    ...
    def initialize(self):
        ...
        self.parser.add_argument('--results_root', type=str, default='', help='If set, save checkpoints and logs to results_root/{name}/ instead of checkpoints_dir/{name}/')
    ...
    def parse(self):
        ...
        file_name = os.path.join(expr_dir, 'opt_{}.txt'.format(self.opt.phase))
        ...
        if self.opt.results_root:
            results_dir = os.path.join(self.opt.results_root, self.opt.name)
            os.makedirs(results_dir, exist_ok=True)
            results_opt = os.path.join(results_dir, 'opt_{}.txt'.format(self.opt.phase))
            with open(results_opt, 'wt') as opt_file:
                opt_file.write('------------ Options -------------\n')
                for k, v in sorted(args.items()):
                    opt_file.write('%s: %s\n' % (str(k), str(v)))
                opt_file.write('-------------- End ----------------\n')
